...
from layers import KGLayer
Model = model.Model


class RotAtte(Model):
    def __init__(self, n_ent, n_rel, in_dim, out_dim, facts, n_heads=1,
                 n_layers=1, input_drop=0.5, negative_rate=10, margin=6.0,
                 epsilon=2.0, batch_size=None, device="cuda"):
        ...
        if n_layers == 2:
            self.a2 = KGLayer(n_ent, n_rel, self.n_heads * out_dim, out_dim, input_drop,
                              margin=margin, epsilon=epsilon, concat=False)
        ...
        if self.n_layers == 1:
            ...
            self.rel_transform = nn.Linear(out_dim, out_dim // 2).to(device)
