...
class KGLayer(nn.Module):
    def __init__(self, n_entities, n_relations, in_dim, out_dim, input_drop=0.5,
        ...
        self.rel_transform = nn.Linear(in_dim, out_dim)
...
class KGLayer2(nn.Module):
    def __init__(self, n_entities, n_relations, in_dim, out_dim, input_drop=0.5,
        ...
        if concat:
            ...
            self.e = nn.Linear(2 * in_dim, out_dim).cuda()
            self.r = nn.Linear(in_dim, out_dim).cuda()

        self.input_drop = nn.Dropout(input_drop)

    def forward(self, triplets):
        # h = self.ent_embed(triplets[:, 0])
        # r = self.rel_embed(triplets[:, 1])
        # t = self.ent_embed(triplets[:, 2])

        # ent_embed = self.e(torch.cat([h, t], dim=1))
        # ent_embed = scatter(ent_embed, triplets[:, 0].unique(), dim=0, reduce="mean")
        # rel_embed = self.r(r)
        # rel_embed = scatter(rel_embed, triplets[:, 1].unique(), dim=0, reduce="mean")

        # return ent_embed, rel_embed

        return self.ent_embed(torch.arange(self.n_entities).cuda()), self.rel_embed(torch.arange(self.n_relations).cuda())
